...
class Detector(torch.nn.Module):
    ...
    def __init__(self, layers=[32, 64, 128], c_in=3, c_out=2, input_normalization: bool = True,
                 skip: bool = True, residual: bool = False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for layers in self.net_conv:
            x = layers(x)
            convlayerskip.append(x)
        convlayerskip.pop(-1)
        skip = False
        for layers in self.net_upconv:
            if skip and len(convlayerskip) > 0:
                x = torch.cat([x, convlayerskip.pop(-1)], 1)
                x = layers(x)
            else:
                x = layers(x)
                skip = self.skip
